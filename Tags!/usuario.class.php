...
class Usuario{
    ...
    public function __construct($usuario, $email, $senha, $conexao)//metodo construtor para o objeto usuario
    {
    ...
    public static function autenticarUsuario($usuario, $senha, $conexao) { //metodo criado apenas para autenticar usuarios pq se manter do jeito que tava dá erro
        $instancia = new self($usuario, null, $senha, $conexao);
        return $instancia;
    }
    ...
    public function salvar(){
        $sql = "INSERT INTO USUARIOS (usuario, email, senha) VALUES (?, ?, ?)"; //usa os pontos de interrogação onde os dados vão entrar
    
        $stmt = $this->conexao->prepare($sql);

        if ($stmt){
            $stmt->bind_param("sss", $this->usuario, $this->email, $this->senha);
            return $stmt->execute();
        }else{
            return false;
        }
